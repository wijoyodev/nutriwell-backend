...
const getRate = async (requestPayload: {
  ...
  const cacheData = getCache(CACHE_KEY.rates+String(requestPayload.destination_postal_code)+String(requestPayload.items[0].quantity));
  if (!cacheData) {
    ...
    if (getResponse) {
      if (getResponse.error) {
        ...
        const saveCache = generateCache(CACHE_KEY.rates+String(destination_postal_code+String(items[0].quantity)), getResponse.pricing, 1200);
        if (!saveCache) Logger.error(`Cache: Set cache with key ${CACHE_KEY.rates+String(destination_postal_code)+String(items[0].quantity)} failed`);
